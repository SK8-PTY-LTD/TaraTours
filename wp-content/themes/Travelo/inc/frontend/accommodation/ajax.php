...
if ( ! function_exists( 'trav_ajax_acc_submit_review' ) ) {
	function trav_ajax_acc_submit_review() {
		...
		if ( ! $booking_data = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . TRAV_ACCOMMODATION_BOOKINGS_TABLE . ' WHERE booking_no=%d AND pin_code=%d', $data['booking_no'], $data['pin_code'] ), ARRAY_A ) ) {
			$result_json['success'] = 0;
			$result_json['result'] = __( 'Wrong Booking Number and Pin Code.', 'trav' );
			wp_send_json( $result_json );
		}

		if ( ! is_array( $booking_data ) || $booking_data['status'] == 0 ) {
			$result_json['success'] = 0;
...
if ( ! function_exists( 'trav_ajax_acc_submit_booking' ) ) {
	function trav_ajax_acc_submit_booking() {
		...
		if ( ! empty( $trav_options['acc_pay_paypal'] ) && ! empty( $booking_data['deposit_price'] ) ) {
			$paypal_payment = true;
		}
		...
		$default_booking_data = array(  'first_name'        => '',
										...
										'exchange_rate'     => 1,
										'deposit_price'		=> 0,
										'deposit_paid'		=> ( $paypal_payment ? 0 : 1 ),
										'date_from'         => '',
										'date_to'           => '',
...
if ( ! function_exists( 'trav_ajax_acc_check_room_availability' ) ) {
	function trav_ajax_acc_check_room_availability() {
		...
		if ( ! $booking_data = $wpdb->get_row( 'SELECT * FROM ' . TRAV_ACCOMMODATION_BOOKINGS_TABLE . ' WHERE booking_no="' . esc_sql( $booking_no ) . '" AND pin_code="' . esc_sql( $pin_code ) . '"', ARRAY_A ) ) {
			$result_json['success'] = 0;
			$result_json['result'] = __( 'Wrong booking number and pin code.', 'trav' );
			wp_send_json( $result_json );
		}
