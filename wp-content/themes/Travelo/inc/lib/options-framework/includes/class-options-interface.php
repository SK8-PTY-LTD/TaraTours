...
class Options_Framework_Interface {
    ...
    static function optionsframework_tabs() {
        ...
        foreach ( $options as $value ) {
            ...
            if ( $value['type'] == "heading" ) {
                ...
                $menu .= '<li><a id="options-group-' . $counter . '-tab" class="of-nav-tab ' . $class . '" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '">' . ( $value['name'] ) . '</a></li>';
            }
        }
        $menu .= '</ul>';
    ...
    static function optionsframework_fields() {
        ...
        foreach ( $options as $value ) {
            ...
            // number input
            case 'number':
                $output .= '<input id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="number" value="' . esc_attr( $val ) . '" />';
                break;
            ...
            case 'select':
                ...
                foreach ($value['options'] as $key => $option ) {
                    $output .= '<option'. selected( $val, $key, false ) .' value="' . esc_attr( $key ) . '">' . ( $option ) . '</option>';
                }
                $output .= '</select>';
                break;
            ...
            case 'typography':
                ...
                if ( $typography_options['sizes'] ) {
                    ...
                    foreach ( $sizes as $i ) {
                        $size = $i . 'px';
                        $font_size .= '<option value="' . esc_attr( $size ) . '" ' . selected( $typography_stored['size'], $size, false ) . '>' . ( $size ) . '</option>';
                    }
                    $font_size .= '</select>';
                }
                ...
                if ( $typography_options['faces'] ) {
                    ...
                    foreach ( $faces as $key => $face ) {
                        $font_face .= '<option value="' . esc_attr( $key ) . '" ' . selected( $typography_stored['face'], $key, false ) . '>' . ( $face ) . '</option>';
                    }
                    $font_face .= '</select>';
                }
            ...
            case 'background':
                ...
                foreach ($repeats as $key => $repeat) {
                    $output .= '<option value="' . esc_attr( $key ) . '" ' . selected( $background['repeat'], $key, false ) . '>'. ( $repeat ) . '</option>';
                }
                $output .= '</select>';
                ...
                foreach ($positions as $key=>$position) {
                    $output .= '<option value="' . esc_attr( $key ) . '" ' . selected( $background['position'], $key, false ) . '>'. ( $position ) . '</option>';
                }
                $output .= '</select>';
                ...
                foreach ($attachments as $key => $attachment) {
                    $output .= '<option value="' . esc_attr( $key ) . '" ' . selected( $background['attachment'], $key, false ) . '>' . ( $attachment ) . '</option>';
                }
                $output .= '</select>';
                $output .= '</div>';
