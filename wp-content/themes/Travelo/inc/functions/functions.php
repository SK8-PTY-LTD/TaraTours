...
if ( ! function_exists( 'trav_wp_title' ) ) {
	function trav_wp_title( $title, $sep ) {
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( __( 'Page %s', 'trav' ), max( $paged, $page ) );
		}
...
if ( ! function_exists( 'trav_enqueue_scripts' ) ) {
	function trav_enqueue_scripts() {
		...
		if ( TRAV_MODE == 'dev' ) {
			$custom_css .= '
				...
				#header.style1 .logo a:after {
					position: absolute; display: block; width: 130px; height: 30px; background: url("http://www.soaptheme.net/wordpress/travelo/wp-content/themes/Travelo/images/logo_txt.png") no-repeat #01b7f2; content: ""; top: 0; right: 0;
				}';
		}
		...
		wp_register_script( 'trav_script_gmap3', TRAV_TEMPLATE_DIRECTORY_URI . '/js/gmap3.min.js', array( 'trav_jquery' ), '3.0', true);
		wp_register_script( 'trav_script_jquery_ui', TRAV_TEMPLATE_DIRECTORY_URI . '/js/jquery-ui.min.js', array( 'trav_jquery' ), '', true );
		wp_register_script( 'trav_script_bootstrap', TRAV_TEMPLATE_DIRECTORY_URI . '/js/bootstrap.min.js', array( 'trav_jquery' ), '3.0', true );
		wp_register_script( 'trav_script_bxslider', TRAV_TEMPLATE_DIRECTORY_URI . '/js/components/jquery.bxslider/jquery.bxslider.min.js', array( 'trav_jquery' ), '', true );
		wp_register_script( 'trav_script_flex_slider', TRAV_TEMPLATE_DIRECTORY_URI . '/js/components/flexslider/jquery.flexslider-min.js', array( 'trav_jquery' ), '', true );
		wp_register_script( 'trav_script_google_map', 'https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false', array(), '3.0', true );
		wp_register_script( 'trav_script_jquery_validate', TRAV_TEMPLATE_DIRECTORY_URI . '/js/jquery.validate.min.js', array( 'trav_jquery' ), '', true );
		wp_register_script( 'trav_script_plugin', TRAV_TEMPLATE_DIRECTORY_URI . '/js/plugin.js', array( 'trav_jquery' ), '', true );
		wp_register_script( 'trav_script_page_loading', TRAV_TEMPLATE_DIRECTORY_URI . '/js/page-loading.js', array( 'trav_jquery' ), '', true );
		wp_register_script( 'trav_script_main_script', TRAV_TEMPLATE_DIRECTORY_URI . '/js/theme-scripts.js', array( 'trav_jquery' ), '', true );
		wp_register_script( 'trav_script_accommodation', TRAV_TEMPLATE_DIRECTORY_URI . '/js/accommodation.js', array( 'trav_jquery' ), '', true );
		wp_register_script( 'trav_script_tour', TRAV_TEMPLATE_DIRECTORY_URI . '/js/tour.js', array( 'trav_jquery' ), '', true );

		wp_enqueue_script( 'trav_jquery' );
		...
		if ( 'dev' == TRAV_MODE ) {
			wp_register_script( 'trav_style_changer', TRAV_TEMPLATE_DIRECTORY_URI . '/js/style-changer.js', array( 'trav_jquery' ), '', true );
		...
		if ( is_singular('tour' ) ) {
			...
			$tour_data['msg_no_booking_page'] = __( 'Please set accommodation booking page on admin/Theme Options/Page Settings', 'trav' );
				...
				<div class="details">
					<?php if ( $list_style == "style1" ) { ?>
						...
						<div class="post-meta single-line-meta vcard">
							...
							<a href="<?php echo esc_url( get_comments_link( $post_id ) ); ?>" class="comment">
								<?php
									$comment_number = get_comments_number( $post_id );
									if ( $comment_number == 0 ) {
										_e( 'No Comments', 'trav' );
									} elseif ( $comment_number == 1 ) {
										esc_html_e( '1 Comment', 'trav' );
									} else {
										echo esc_html( $comment_number ) . ' ';
										esc_html_e( 'Comments', 'trav' );
									}
								?>
							</a>
						</div>
 ...
 * function to make it enable d/m/Y strtotime
 */
if ( ! function_exists( 'trav_get_phptime' ) ) {
