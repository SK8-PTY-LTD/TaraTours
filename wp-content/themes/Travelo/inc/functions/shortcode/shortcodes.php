...
class TravShortcodes {
	...
	* **************************************************************** */
	...
	function shortcode_toggles( $atts, $content = null ) {
		...
		$result .= '<div class="toggle-container' . esc_attr( $class ) . '" id="accordion' . esc_attr( $this->accordion_id ) . '" ' . $img_atts . ' >';
		$result .= do_shortcode( $content );
		$result .= '</div>';
	...
	* **************************************************************** */
	function shortcode_toggle( $atts , $content = null ) {
		...
		$result = '';
		$result .= '<div class="panel ' . esc_attr( $this->toggle_style . $class ) . '">';
	...
	* **************************************************************** */
	function shortcode_social_links($atts, $content = null) {
		...
		$variables = array( 'style'=>'style1', 'linktarget'=>'_blank', 'class'=>'' ); //available style values ( style1|style2 )
	...
	* **************************************************************** */
	function shortcode_testimonial($atts, $content = null) {
		...
		if ( $this->testimonial_style != "style3" ) {
			...
			$result .= '<li' . $class . '>';
			$result .= '<div class="author-section"><a href="' . esc_url( $author_link ) . '"><img src="' . esc_url( $author_img_url ) . '" alt="' . esc_attr( $author_img_alt ) . '" width="' . esc_attr( $this->author_img_size ) . '" height="' . esc_attr( $this->author_img_size ) . '" /></a></div>';
	...
	* **************************************************************** */
	function shortcode_icon_box( $atts, $content = null ) {
		...
		if ( empty( $style ) ) {
			...
			$result = '<div class="icon-box ' . esc_attr( $style ) . ' counters-box' . esc_attr( $class ) . '"><div class="numbers"><i class="' . esc_attr( $icon ) . esc_attr( $icon_class ) . '"></i>';
			$result .= '<span class="display-counter" data-value="' . esc_attr( $number ) . '">' . esc_attr( $number ) . '</span></div>';
			$result .= do_shortcode( $content );
			$result .= '</div>';
		} elseif ( $style == 'style11' ) {
			$result = '<div class="icon-box ' . esc_attr( $style . $class ) . '"><div class="icon-wrapper"><i class="' . esc_attr( $icon . $icon_class ) . '"></i></div>';
			$result .= '<div class="details"><h4 class="m-title">';
			$result .= do_shortcode( $content );
			$result .= '</h4></div></div>';
		} elseif ( ( $style == 'style5' ) || ( $style == 'style6' ) || ( $style == 'style7' ) ) {
			$result = '<div class="icon-box ' . esc_attr( $style . $class ) . '"><i class="' . esc_attr( $icon . $icon_class ) . '"></i><div class="description">';
			$result .= do_shortcode( $content );
			$result .= '</div></div>';
		} else {
			$result = '<div class="icon-box ' . esc_attr( $style . $class ) . '"><i class="' . esc_attr( $icon . $icon_class );
	...
	* **************************************************************** */
	function shortcode_parallax_block( $atts, $content = null ) {
		...
		if ( empty( $ratio ) || ! is_numeric( $ratio ) ) $ratio = 0.5;
	...
	function shortcode_accommodations( $atts ) {
		...
		if ( $style == 'style4' ) {
			...
			if ( $slide == 'false' ) {
				...
				$def_before_list = '<div class="block image-carousel style2 flexslider" data-animation="slide" data-item-width="270" data-item-margin="30"><ul class="slides hotel image-box listing-' . esc_attr( $style ) . '">';
	...
	* **************************************************************** */
	function shortcode_tours( $atts ) {
		...
		if ( $slide == 'false' ) {
			...
			$def_before_list = '<div class="block image-carousel style2 flexslider" data-animation="slide" data-item-width="270" data-item-margin="30"><ul class="slides tour-packages image-box listing-' . esc_attr( $style ) . '">';
			$def_after_list = '</ul></div>';
			$def_before_item = '<li>';
			$def_after_item = '</li>';
		}
	...
	* **************************************************************** */
	function shortcode_latest_tours( $atts ) {
		...
		foreach ( $tours as $tour ) {
			...
			$result .= '<label class="price-wrapper"><span class="price-per-unit">' . esc_html( trav_get_price_field( $min_price ) ) . '</span> ' . __( 'avg/night', 'trav' ) . '</label>';
			$result .= '</div>';
			$result .= '</article>';
		}
		$result .= '</div>';
	...
	function shortcode_posts( $atts ) {
		...
		if ( $style == 'style3' ) {
			...
			if ( $slide == 'false' ) {
				...
				$def_after_list = '</div>';
				$def_before_item = '<div class="col-sm-6 col-md-3">';
				$def_after_item = '</div>';
			} else {
		...
		if ( $type == 'selected' ) {
			...
			$posts = get_posts('posts_per_page=' . $count . '&meta_key=trav_count_post_views&orderby=meta_value_num&order=DESC&suppress_filters=0');
		}
	...
	* **************************************************************** */
	function shortcode_dashboard( $atts, $content = null ) {
		ob_start();
		trav_get_template( 'dashboard.php', '/templates' );
		$output = ob_get_contents();
		ob_end_clean();
		return $output;
	}
	...
	* **************************************************************** */
	function shortcode_bgslider( $atts, $content = null ) {
		$variables = array( 'class' => '', 'img_urls' => '' );
		extract( shortcode_atts( $variables, $atts ) );
		$id = empty( $id )?'':( ' id="' . esc_attr( $id ) . '"' );
		$class = empty( $class )?'':( ' ' . $class );
		$ul_class = empty( $ul_class )?'':( ' ' . $ul_class );
		...
		$result = '';
		$result .= '<div' . $id . ' class="slideshow-bg full-screen' . esc_attr( $class ) . '">';
	...
	* **************************************************************** */
	function shortcode_promo_box( $atts, $content = null ) {
		$variables = array( 'class' => '', 'img_src' => '', 'img_alt' => 'promobox-image', 'img_height' => '', 'img_width' => '', 'content_section_width' => '', 'img_section_width' => '', 'type' => 'type1', 'animation_type' => '', 'animation_duration' => '', 'animation_delay' => '', 'img_class' => '' );
		extract( shortcode_atts( $variables, $atts ) );
		$class = empty( $class )?'':( ' ' . $class );
	...
	* **************************************************************** */
	function shortcode_search_form( $atts, $content = null ) {
		...
		global $trav_options, $search_max_rooms, $search_max_adults, $search_max_kids;

		ob_start();
		if ( $post_type == 'accommodation' && empty( $content ) ) { ?>
			<?php if ( $style == 'style2' ) : ?>
				<div class="search-box">
					<form role="search" method="get" id="searchform" class="acc-searchform2" action="<?php echo esc_url( home_url( '/' ) ); ?>">
						...
						<div class="row">
							...
							<div class="form-group col-sm-6 col-md-3">
								...
								<div class="row">
									...
									<?php endif; ?>
									<div class="col-xs-6">
										<input type="text" name="max_price" class="input-text full-width" placeholder="<?php echo sprintf( __( 'Max Budget (%s)', 'trav'), $trav_options['def_currency'] ) ?>" />
									</div>
								</div>
							</div>
							<div class="form-group col-sm-6 col-md-2">
	...
	* **************************************************************** */
	function shortcode_person( $atts, $content = null ) {
		$variables = array( 'style' => 'style1', 'img_src' => '', 'img_alt' => 'person-photo', 'img_width' => '', 'img_height' => '', 'link' => '#', 'class' => '' );
		extract( shortcode_atts( $variables, $atts ) );
		$class = empty( $class )?'':( ' ' . $class );
		...
		'<script type="text/javascript">
			jQuery(document).ready( function() {
				jQuery("#map' . $map_id . '").gmap3({
					map: {options: {' . $center_str . $zoom . $type_control . $nav_control . $scrollwheel . $street_view_control . '}},'
					. $marker_str
				. '});
			});
		</script>';
