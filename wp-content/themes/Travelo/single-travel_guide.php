					...
					<div id="main" class="col-md-9">
						<div class="tab-container style1" id="travel-guide">
							...
							<?php
									...
									if ( $order_by == 'price' ) {
										$args = array(
											...
											'meta_query' => array(
												'relation' => 'AND',
												array(
													'key'     => 'trav_accommodation_avg_price',
													'value'   => '0',
													'compare' => 'NOT EXISTS'
												),
												array(
										...
										$acc_without_price = get_posts( $args );
										if ( $order == 'ASC' ) {
											$accs = array_merge( $acc_without_price, $accs );
										} else {
											$accs = array_merge( $accs, $acc_without_price );
										}
									} elseif ( $order_by == 'rating' ) {
										$args = array(
											'post_type' => 'accommodation',
										...
										$acc_without_review = get_posts( $args );
										if ( $order == 'ASC' ) {
											$accs = array_merge( $acc_without_review, $accs );
										} else {
											$accs = array_merge( $accs, $acc_without_review );
										}
									}
									$count = count( $accs );
								...
								if ( empty( $trav_options['disable_tour' ] ) ) :
									...
									if ( $order_by == 'price' ) {
										...
										$tours_without_price = get_posts( $args );
										if ( $order == 'ASC' ) {
											$tours = array_merge( $tours_without_price, $tours );
										} else {
											$tours = array_merge( $tours, $tours_without_price );
										}
									}
									$count = count( $tours );
									$tour_offset = ( $tour_page - 1 ) * $tour_per_page;
									for ( $i = $tour_offset; $i < $tour_page * $tour_per_page; $i++ ) {
										if ( ! isset( $tours[$i] ) ) break;
										$tour = $tours[$i];
										$tour_list[] = $tour->ID;
									}
								endif;
							...
							<div class="tab-content">
										...
										<div class="hotel-list list-wrapper">
											<?php
												if ( $current_view == 'block' ) {
													...
													echo '<div class="listing-style3 hotel">';
													$before_article = '';
													$after_article = '';
												}
								...
								<?php } ?>
								<?php
								if ( ! empty( $tours ) && isset( $tours[$tour_offset] ) ) {
									?>
									<div class="tab-pane gray-bg fade<?php echo esc_attr( $active_class ); $active_class=''; ?>" id="travel-guide-tours">
										...
										<div class="hotel-list list-wrapper">
											<?php
												if ( $current_view == 'block' ) {
													...
													echo '<div class="tour-packages listing-style3">';
													$before_article = '';
													$after_article = '';
												}
										...
										<?php if ( ! empty( $trav_options['ajax_pagination'] ) ) { ?>
											<?php if ( count( $tour_list ) >= $tour_per_page ) { ?>
												<a href="<?php echo esc_url( add_query_arg( array( 'tour_page' => ( $tour_page + 1 ) ) ) ); ?>" class="uppercase full-width button btn-large btn-load-more-tours" data-view="<?php echo esc_attr( $current_view ); ?>" data-search-params="<?php echo wp_kses_post( http_build_query($_GET, '', '&amp;') );?>"><?php echo __( 'load more listing', 'trav' ) ?></a>
											<?php } ?>
										<?php } else {
											unset( $_GET['tour_page'] );
											$pagenum_link = strtok( $_SERVER["REQUEST_URI"], '?' ) . '%_%';
											$total = ceil( $count / $tour_per_page );
											$args = array(
												'base' => $pagenum_link, // http://example.com/all_posts.php%_% : %_% is replaced by format (below)
												'total' => $total,
												'format' => '?tour_page=%#%',
